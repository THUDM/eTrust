...
int main(int argc, char *argv[]) {
    ...
    string fileName;
    if (argc < 2) {
        fprintf(stderr, "Usage: ./dtTrust-fast <filename>\n");
        return -1;
    } else if (argc == 2) {
    ...
    // for (int group = 0; group < FOLD_NUM; ++group)
    {
        ...
        //         continue;
        //     }
        //     nodes[i].known = nodes[i].valid = nodes[i].test = false;
    ...
    // fprintf(stdout, "pre_AVG:\n");
    // fprintf(stdout, "  P = %.2f%%  R = %.2f%%  F1 = %.2f%%\n", pre_result.P * 100, pre_result.R * 100, pre_result.F1 * 100);
    // fprintf(stdout, "\n");

    // fprintf(stdout, "AVG:\n");
    // fprintf(stdout, "  P = %.2f%%  R = %.2f%%  F1 = %.2f%%\n", result.P * 100, result.R * 100, result.F1 * 100);
    // fprintf(stdout, "\n");
